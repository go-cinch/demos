...
func (*Module) Name() string { return "/whitelist" }
...
func (m *Module) get(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		server.WriteError(w, r, http.StatusBadRequest, apperror.WhitelistID)
		return
	}
...
func (m *Module) update(w http.ResponseWriter, r *http.Request) {
	...
	var input UpdateWhitelistInput
	decoder := json.NewDecoder(http.MaxBytesReader(w, r.Body, 1<<20))
	decoder.DisallowUnknownFields()
	if err := decoder.Decode(&input); err != nil {
		server.WriteError(w, r, http.StatusBadRequest, apperror.InvalidBody)
...
func (m *Module) list(w http.ResponseWriter, r *http.Request) {
	...
	query := r.URL.Query()
	input := ListWhitelistsInput{Resource: query.Get("resource")}
	if values, provided := query["category"]; provided {
		if len(values) != 1 {
			server.WriteError(w, r, http.StatusBadRequest, apperror.CategoryType)
			return
		}
		parsed, err := strconv.ParseInt(values[0], 10, 16)
		if err != nil || parsed < 0 || parsed > 1 {
			server.WriteError(w, r, http.StatusBadRequest, apperror.Category)
			return
		}
		value := int16(parsed)
	...
	if err != nil {
		slog.ErrorContext(r.Context(), "list whitelist entries failed: "+err.Error())
		server.WriteError(w, r, http.StatusInternalServerError, apperror.Internal)
		return
	}
	server.WriteOK(w, value)
